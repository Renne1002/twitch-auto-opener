...
class UploadStateStore:
    ...
    def mark_uploaded(self, file_key: str, *, video_id: str) -> None:
    ...
    def mark_failed(self, file_key: str, *, error_code: str, error_reason: str) -> None:
        failed_files = self._state.setdefault("failed_files", {})
        current = failed_files.get(file_key, {})
        failures = int(current.get("failures", 0)) + 1
        delay_seconds = min(3600, 30 * (2 ** (failures - 1)))
